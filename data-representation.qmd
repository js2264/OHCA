...
This requires a bit of wrangling, to mimmic an overlap between two `GRanges` objects: 

```{r}
gi <- swapAnchors(gi) # ----- Make sure anchors are correctly sorted
gi <- sort(gi) # ----- Make sure interactions are correctly sorted
gi <- gi[!is.na(pairdist(gi))] # ----- Remove interchromosomal interactions
spanning_gi <- GRanges(
...
::: {.callout-warning icon='false'}
Remember: `ContactFile` objects are only **connections** to a disk-stored HiC file. 
...
interactions as `GInteractions`. 

### Creating a `HiCExperiment` object

#### `import`ing a `ContactFile` 
...
whatever score the end-user whant to attribute to each interaction 
...
- For `HicFile` and `HicproFile`, `import` will return a `HiCExperiment` as well: 
...
- `resolution`: This can be used to choose which resolution to parse the contact matrix at. 
:::
...
The imported genomic interactions are available using the `interactions` function
and are returned as a `GInteractions` object. 

```{r}
interactions(hic)
```

Note that in the `hic` object, we only have `11` interactions stored. This is 
because the `focus` is rather specific: it focuses on the interactions between 
small segments of `III` and `XV` chromosomes. 
...
If extra scores have to be added (e.g. to describe the "expected" interaction 
frequency for each interaction stored in the `HiCExperiment` object), it can be 
done using the `scores()<-` function: 
...
### Transforming `HiCExperiment` objects

Convenient "coercing" functions exists to transform data stored as a `HiCExperiment`
into another class. 

- `as("GInteractions")`: virtually the same than `interactions`
...
and discard all other information, e.g. available `resolutions`, 
associated `metadata`, `pairsFile` or `topologicalFeatures`. 
:::
