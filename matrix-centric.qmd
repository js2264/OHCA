...
**Matrix-centrix** analyses consider a `HiCExperiment` object from the 
"matrix" perspective, in order to perform a range of matrix-based operations. 
This encompasses: 
...
(@Cournac2012Dec, @Imakaev2012Oct). This is generally done directly on the 
...
of chromatin into two A/B compartments (@Lieberman-Aiden2009Oct). 
...
conformation (@Duan2010May). An example of autocorrelation map 
...
```{r}
div_contacts <- divide(hic2, by = hic) 
div_contacts
```
...
cowplot::plot_grid(
    plotMatrix(hic2, compare.to = hic, limits = c(-4, -1)), 
    plotMatrix(
        div_contacts, 
